...
export class SwappingService {
  ...
  async acceptSwap(dto: { currentUserId: string; remoteUserId: string }) {
    try {
      ...
      if (!currentUser.lookingFor.includes(remoteUser.alloted) || !remoteUser.lookingFor.includes(currentUser.alloted)) {
        throw new BadRequestException("Requested swap is not valid");
      }
